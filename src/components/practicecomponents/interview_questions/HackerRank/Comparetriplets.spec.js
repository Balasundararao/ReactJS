...
function compareTriplets(a, b) {
    let result = {acounter: 0, bcounter: 0}; 
    if( a.length != b.length) {
        return [];
    }
    let total_points =  a.reduce( (acc,val, index )  => {
        if( a[index] > b[index]  ) {
            acc["acounter"]++;
    ...
    return Object.values( total_points );
}
//console.log( compareTriplets([17,28,30], [99,16,8]) );

test('Compare Triplets... [5,6,7], [3,6,10] ', () => {
    expect( compareTriplets([5,6,7], [3,6,10])).toEqual([1,1])
})

test('Compare Triplets... [17,28,30], [99,16,8] ', () => {
